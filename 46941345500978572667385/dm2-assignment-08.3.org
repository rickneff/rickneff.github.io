...
#+name: main
#+BEGIN_SRC elisp :results output :var start=0 end=100
...
* TODO Note the Constraints
  This week\rsquo{}s assignment gives you another chance to solve problems using
  limited or constrained resources. Python is a language that in some ways has
  fewer constraints than elisp. For example, see some RSA calculations much more
  easily done:
...
#+BEGIN_SRC elisp :results raw 
  ...
  (let* ((max-specpdl-size most-positive-fixnum)
    ...
    (list (math-format-number c) m-again (= m m-again)))
#+END_SRC

#+RESULTS:
(8966759170151112623516383 5179195214304 t)
